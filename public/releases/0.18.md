...
If you're not able to find an answer to your problems you may either contact QuarkX support bot over telegram or send us an email.

## 4. Price Charts

Token price charts now obtained directly from Uniswap smart contracts giving users real time token market rates.

Prior to that QuarkX relied on external services to show the latest prices for various tokens. It was not uncommon for these services to show delayed prices which were no longer valid.
...
Android users are now able to user the preferred keyboard when interacting with the input fields within QuarkX wallet app. For security purposes, external keyboards were disabled by default.
