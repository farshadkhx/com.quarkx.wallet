# How QuarkX recommends transaction fee for Bitcoin?

Well engineered non-custodial wallets incl. QuarkX are able to estimate the required transaction fee value in order for the Bitcoin transaction to be included within the next block.
...
Keep in mind that QuarkX fee recommendation service merely makes an educated guess based on the analysis of the network activity. There is no way to guarantee that the recommended setting would always work as expected.
